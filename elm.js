...
Elm.Multiplication.make = function (_elm) {
   ...
   if (_elm.Multiplication.values) return _elm.Multiplication.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var generateMultiplication = function (seed0) {
      var _p0 = A2($Random.generate,A2($Random.$int,0,10),seed0);
      var first = _p0._0;
      var seed1 = _p0._1;
      var _p1 = A2($Random.generate,A2($Random.$int,0,10),seed1);
      var second = _p1._0;
      var seed2 = _p1._1;
      return {ctor: "_Tuple2",_0: {ctor: "_Tuple2",_0: first,_1: second},_1: seed2};
   };
   var compareInputWithMultiplication = F2(function (multipliction,userInput) {
      var _p2 = $String.toInt(userInput);
      if (_p2.ctor === "Ok") {
            return _U.eq(_p2._0,$UI.resultOfMultiplication(multipliction));
         } else {
            return false;
         }
   });
   var handleInput = F2(function (userInput,model) {
      var _p3 = A2(compareInputWithMultiplication,model.multiplication,userInput);
      if (_p3 === true) {
            var _p4 = generateMultiplication(model.currentSeed);
            var multiplication = _p4._0;
            var newSeed = _p4._1;
            return _U.update(model,
            {counter: model.counter + 1,multiplication: multiplication,score: model.score + 1,userInput: "",currentSeed: newSeed,gameState: $Model.Running});
         } else {
            return _U.update(model,{userInput: userInput});
         }
   });
   var updateGame = F2(function (action,model) {
      var _p5 = action;
      switch (_p5.ctor)
      {case "Tick": return _U.update(model,{counter: model.counter - 1,currentSeed: $Random.initialSeed($Basics.round(_p5._0))});
         case "Input": return A2(handleInput,_p5._0,model);
         default: return model;}
   });
   var updateRunning = F2(function (action,model) {
      var _p6 = _U.cmp(model.counter,0) < 1;
      if (_p6 === false) {
            return A2(updateGame,action,model);
         } else {
            return _U.update(model,{gameState: $Model.Stopped});
         }
   });
   var updateStopped = F2(function (action,model) {
      var _p7 = action;
      if (_p7.ctor === "Reset") {
            return _U.update(model,{counter: 9,userInput: "",score: 0,gameState: $Model.NotStarted});
         } else {
            return model;
         }
   });
   var updateNotStarted = F2(function (action,model) {
      var _p8 = action;
      switch (_p8.ctor)
   ...
   var update = F2(function (action,model) {
      ...
      switch (_p9.ctor)
      {case "NotStarted": return A2(updateNotStarted,action,model);
         case "Running": return A2(updateRunning,action,model);
         default: return A2(updateStopped,action,model);}
   });
   var modelToInputId = function (model) {    return "input";};
   var ticks = A2($Signal.map,function (_p10) {    var _p11 = _p10;return $Action.Tick(_p11._0);},$Time.timestamp($Time.every($Time.second)));
   var userActionsMailbox = $Signal.mailbox($Action.Tick(0));
   var actions = A2($Signal.merge,ticks,userActionsMailbox.signal);
   var modelUpdates = A3($Signal.foldp,update,$Model.initialModel,actions);
   var focusElement = Elm.Native.Port.make(_elm).outboundSignal("focusElement",function (v) {    return v;},A2($Signal.map,modelToInputId,modelUpdates));
   var main = A2($Signal.map,$UI.view(userActionsMailbox.address),modelUpdates);
